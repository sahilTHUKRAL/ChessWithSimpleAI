...
// Internal Square class for Each Individual Squares 
	class Square  extends JButton{
		...
		public Square(Square s){
			...
			if(s.piece!=null) {
			if(colour ==1) {
				this.piece = pieceSetBlack.CreatePiece(s.piece);
			}
			else {
				this.piece = pieceSetWhite.CreatePiece(s.piece);
			}
			}
			//this.piece= new PieceSet.Piece(s.piece);
			/// issue is here 
		}
...
//Setting Up the Rest of the board with empty Squares	
	...
	private Board.Square[][] copyBoard(Board b) {
		Board.Square[][] cBoard = new Board.Square[8][8];
		for(int i =0;i<8;i++) {
			for(int j=0;j<8;j++) {
				cBoard[i][j]= new Square(b.board[i][j]);
			}
		}
		return cBoard;
	}
	public Board(Board b,Game game) {
		this.colourOfSquare= b.colourOfSquare;
		this.turn= b.turn;
		//this.a = =new Player(game);
		//this.b= new Player(game);
		
		this.b = b.b;
		//this.pieceSetBlack= new PieceSet(colour, board, player);
		this.pieceSetWhite= b.pieceSetWhite;
...
/// Printing the Board On Console/////////////////////////////////////////
	...
	private void swapSquares(Square s1,Square s2) {
	...
	public void turnTheBoard(){
		...
		for(int i=low_x;i<=high_x;i++) {
			...
			k++;
			for(int j=low_y;j<=high_y;j++) {
				swapSquares(board[i][j],board[7-i][7-j]);
			}
			}
		}
	}
...
////-----------------------------Temp Moves in AI conputing for best move avalible-----------------------/////////////////////////////////////
	public boolean tempmoveForAI(Square s1,Square s2, PieceSet.Piece piece) {
...
////-----------------------------Temp Moves in checking for check -----------------------/////////////////////////////////////

public boolean tempmoveForCheck(Square s1,Square s2, PieceSet.Piece piece) {
		//exchangePiece(s1, s2);
		Square temp1 = new Square(s1.piece,s1.colour,s1.x,s1.y);
		if(temp1.piece!=null) {
			temp1.piece.x=s1.piece.x;
	...
	public void Resetboard(Game game) {
		 {
			 //ArrayList<Board.Square> buttons = new  ArrayList<Board.Square>();
		        for(int l = 0; l <9; l++){
		        for(int m = 0; m <9; m++)
		   	        {
		        	if(l>0 && m>0) {
